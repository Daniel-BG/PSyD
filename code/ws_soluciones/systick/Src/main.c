...
void TIM3_IRQHandler(void) {
    ...
    if (TIM_CHECK_FLAG(TIM3)) {
        ...
        if (GPIO_PIN_READ(GPIOA, 0) == GPIO_STATE_ONE) {
        	uint32_t curr_tick = GetTick();
        	if (curr_tick - prev_press < 500) { //if under 500ms, double click
        		// Toggle Green LED on double click
        		GPIO_PIN_TOGGLE(GPIOB, 1);
        	} else {
        		// Always Toggle red led
        		GPIO_PIN_TOGGLE(GPIOA, 7);
        	}
        	prev_press = curr_tick;
        }
