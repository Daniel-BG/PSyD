...
void LCD_Startup_Seq() {
	...
	Delay_ms(5);   /* Reset signal asserted during 5ms  */
	GPIO_PIN_WRITE(GPIOH, 7, GPIO_STATE_ONE);
	Delay_ms(10);  /* Reset signal released during 10ms */
	GPIO_PIN_WRITE(GPIOH, 7, GPIO_STATE_ZERO);
	Delay_ms(20);  /* Reset signal asserted during 20ms */
	GPIO_PIN_WRITE(GPIOH, 7, GPIO_STATE_ONE);
	Delay_ms(10);  /* Reset signal released during 10ms */
}
...
/* Configure PA0 as digital input for the User/BOOT button */
void UserButton_Init(void) {
	RCC_AHB1ENR->bits.GPIOAEN = 1;
	GPIO_CONFIG(GPIOA, 0, GPIO_MODE_INPUT, GPIO_OTYPE_PP, GPIO_OSPEED_LOW, GPIO_PUPD_NONE, 0);
}

/* Returns 1 if button is held down, 0 otherwise */
uint8_t UserButton_IsPressed(void) {
    return GPIO_PIN_READ(GPIOA, 0);
}



void showSquare(int x, int y, int size, uint16_t color) {
	/* Show Green Square */
	for (int i = x - size; i < x + size; i++)
		for (int j = y - size; j < y + size; j++)
			ST7789H2_WritePixel(i, j, color);
}
...
int main(void)
{
	...
	ST7789H2_INIT();
	/* Clear the LCD */
	showSquare(120, 120, 120, LCD_COLOR_WHITE);
	// Init comm with TS
	I2C3_Init();
	TS_INT_Init();
	...
	if (Flash_ReadTouch(&last_x, &last_y)) {
		showSquare(last_x, last_y, 5, LCD_COLOR_GREEN);
	} else {
		showSquare(120, 120, 5, LCD_COLOR_YELLOW);
	}
	...
	while (1) {
		...
		if (g_ts_interrupt_flag) {
			...
			if (FT3X67_GetTouchCoordinates(&current_touch)) {
				...
				last_x = current_touch.y;
				last_y = 240-current_touch.x;
				ST7789H2_WritePixel(last_x, last_y, LCD_COLOR_RED);
			}
		}
