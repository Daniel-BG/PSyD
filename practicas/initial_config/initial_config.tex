...
\section{Configuración del entorno de programación}
    ...
    \subsection{Descarga del material de la asignatura}\label{sub:download}
    ...
    \subsection{Conexión de la placa a la máquina virtual}\label{sub:boardconnect}
    ...
    \subsection{Uso en los laboratorios de la facultad}

        En los laboratorios de la facultad, la máquina está instalada en \textbf{Linux}, por tanto el primer paso es entrar en ese sistema operativo (si está arrancado en windows, tienes que reiniciar el ordenador, y elegir Linux al arrancar). Una vez dentro, hay varios usuarios, elige el \textbf{usuario} normal, NO el de VMs. Ya iniciada la sesión, tendrás que abrir una terminal, y para lanzar la máquina escribir:

        \begin{lstlisting}[language=bash]
            > VM_PsyD_debian.sh
            # si no sale el nombre exactamente igual, escribir solo VM
            # en la terminal y pulsar "tab" para ver las opciones disponibles
        \end{lstlisting}

        Tras esto, se lanzará la máquina virtual con las herramientas. A continuación puedes abrir otra terminal dentro, y en el directorio de tu elección (Se recomienda la carpeta \texttt{Documentos}), descargar el repositorio con el contenido de la asignatura siguiendo el \Cref{sub:download}. Finalmente, para conectar la placa, sigue el \Cref{sub:boardconnect}.
