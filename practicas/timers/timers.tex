...
\ifSubfilesClassLoaded{\setcounter{numpractica}{4}}{}

\practica{Configuración de Timers}
    ...
    \subsection{Desarrollo de la práctica}
        ...
        Y por último tendremos que activar los bits correspondientes en \REG{APB1ENR} (\REFMAN[5.3.13]), y adicionalmente activar en \REG{NVIC} (\REFMAN[9.1.2]) las líneas de interrupción adecuadas:
        ...
        \begin{tikzpicture}[
            ...
            \node (tim2_toggle) [block, below=0.6cm of tim2_start] {1. Limpiar Flag SR\\2. Conmutar LED Verde\\(PB1)};
            ...
            \node (ext0_actions) [block, below=0.6cm of ext0_chk] {
                ...
                3. Reiniciar \texttt{CNT} = 0 en \texttt{TIM3}\\
    ...
    \subsection{Para hacer más}
        Aunque hay varios temporizadores (14 en concreto en el caso de \MCU (\REFMAN[18,19,20])) para sistemas complejos con necesidad de medir una multitud de intervalos, pueden quedarse cortos. Para solventar este problema, cada temporizador es capaz de generar interrupciones en varios modos diferentes y con varios contadores distintos. Como todas estas interrupciones comparten ciertos aspectos de configuración, no podrá haber demasiada diferencia entre los tiempos que midan, aún así da mucha versatilidad. Investiga cómo puedes hacer que haya varias interrupciones diferentes desde un mismo temporizador (\REFMAN[19]). (\textbf{NOTA:}) las interrupciones vendrán por la misma línea, y habrá que averiguar, programáticamente, qué parte del temporizador las causó (\REFMAN[19.4.4,19.4.5]).
